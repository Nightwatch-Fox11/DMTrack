...
from lib.models.layers.adapter import Bi_direct_adapter, Tem_adapter, G_adapter
...
class CEABlock(nn.Module):
    def __init__(self, dim, num_heads, mlp_ratio=4., qkv_bias=False, drop=0., attn_drop=0.,
        ...
        self.adap2_t = G_adapter()
        self.adap_tem1 = Tem_adapter()
        self.adap_tem2 = Tem_adapter()

    def forward(self, x, xi, global_index_template, global_index_templatei, global_index_search, global_index_searchi,
                mask=None, ce_template_mask=None, keep_ratio_search=None):
        x = x + self.adap_tem1(x)
        xi = xi + self.adap_tem2(xi)
        # xi = xi + self.adap_tem1(xi)
        ...
        adapt_for_x, adapt_for_xi = self.adap2_t(self.norm2(x), self.norm2(xi))
        x = x + self.drop_path(self.mlp(self.norm2(x))) + self.drop_path(adapt_for_x)  ###-------adapter
        xi = xi + self.drop_path(self.mlp(self.norm2(xi))) + self.drop_path(adapt_for_xi)  ###-------adapter
