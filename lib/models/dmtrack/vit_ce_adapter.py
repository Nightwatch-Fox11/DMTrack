...
from ..layers.attn_adapt_blocks import CEABlock  ##DMTrack
...
class VisionTransformerCE(VisionTransformer):
    ...
    def forward_features(self, z, x, mask_z=None, mask_x=None,
        ...
        z = torch.stack(z, dim=1)  # (B, Tz, 6, H, W)
        _, T_z, C_z, H_z, W_z = z.shape
        # rgb_img
        x_rgb = x[:, :3, :, :]
        # z_rgb = z[:, :3, :, :]
        z_rgb = z[:, :, :3, :, :].flatten(0, 1)  # (B * Tz, 3, H, W)
        # depth thermal event images
        x_dte = x[:, 3:, :, :]
        # z_dte = z[:, 3:, :, :]
        z_dte = z[:, :, 3:, :, :].flatten(0, 1)  # (B * Tz, 3, H, W)
        # overwrite x & z
        x, z = x_rgb, z_rgb
        xi, zi = x_dte, z_dte
        ...
        if T_z > 1:  # multiple memory frames
            ...
            z = z.flatten(1, 2)
            zi = zi.view(B, T_z, -1, zi.size()[-1]).contiguous()
            zi = zi.flatten(1, 2)  # [Batch size, T_z * N, 768]
