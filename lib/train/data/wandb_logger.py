...
class WandbWriter:
    def __init__(self, exp_name, cfg, output_dir, cur_step=0, step_interval=0):
        ...
        wandb.init(project="MMTrack", name=exp_name, config=cfg, dir=output_dir, resume=False)

    def write_log(self, stats: OrderedDict, epoch=-1, wandb_step=-1):
        self.step += 1
        for loader_name, loader_stats in stats.items():
            ...
            if wandb_step != -1:
                self.wandb.log(log_dict, step=wandb_step)
            else:
