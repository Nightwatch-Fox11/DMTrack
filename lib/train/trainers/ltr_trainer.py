    ...
    def cycle_dataset(self, loader):
        ...
        for i, data in enumerate(loader, 1):
            ...
            if self.wandb_writer is not None and i % self.settings.print_interval == 0:
                # prev_train_epoch = self.epoch - 1
                # prev_val_epoch =
                # log_times_per_epoch = loader.__len__() // self.settings.print_interval
                # world_size = get_world_size()
                # wandb_step = ((self.epoch - 1) * log_times_per_epoch + (
                #             i // self.settings.print_interval)) * batch_size * world_size
                wandb_step = -1
                if self.settings.local_rank in [-1, 0]:
                    self.wandb_writer.write_log(self.stats, self.epoch, wandb_step)
