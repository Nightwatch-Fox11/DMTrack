...
from lib.train.actors import DMTrackActor
# for import modules
import importlib
...
def run(settings):
    settings.description = 'Training script for dmtrack'
    ...
    # Create network
    if settings.script_name == "dmtrack":
        net = build_dmtrack(cfg)
    else:
    ...
    if settings.script_name == "dmtrack":
        ...
        actor = DMTrackActor(net=net, objective=objective, loss_weight=loss_weight, settings=settings, cfg=cfg)
    else:
        raise ValueError("illegal script name")
