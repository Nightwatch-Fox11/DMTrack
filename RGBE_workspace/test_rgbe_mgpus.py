...
def genConfig(seq_path, set_type):
    if set_type == 'VisEvent':
        RGB_img_list = sorted([seq_path + '/vis_imgs/' + p for p in os.listdir(seq_path + '/vis_imgs') if
                               os.path.splitext(p)[1] == '.bmp'])
        E_img_list = sorted([seq_path + '/event_imgs/' + p for p in os.listdir(seq_path + '/event_imgs') if
                             os.path.splitext(p)[1] == '.bmp'])
...
def run_sequence(seq_name, seq_home, dataset_name, yaml_name, num_gpu=1, epoch=60, debug=0, script_name='prompt'):
    ...
    if script_name == 'dmtrack':
        ...
        tracker = DMTrack_RGBE(tracker=mmtrack)
...
if __name__ == '__main__':
    ...
    if dataset_name == 'VisEvent':
        seq_home = '/data/datasets/visevent/test_subset'
