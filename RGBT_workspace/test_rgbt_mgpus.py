...
from lib.test.tracker.dmtrack import DMTrack
import lib.test.parameter.dmtrack as rgbt_adapter_params
...
def run_sequence(seq_name, seq_home, dataset_name, yaml_name, num_gpu=1, epoch=300, debug=0, script_name='adapter'):
    ...
    if script_name == 'dmtrack':
        params = rgbt_adapter_params.parameters(yaml_name, epoch)
        mmtrack = DMTrack(params)  # "GTOT" # dataset_name
        tracker = DMTrack_RGBT(tracker=mmtrack)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--script_name', type=str, default='adapter',
                        help='Name of tracking method(ostrack, adapter, ftuning).')
    parser.add_argument('--yaml_name', type=str,
                        default='ostrack_ce_ep60_adapter_iv21b_wofovea_8_onlylasher_2xa100_rgbt',
